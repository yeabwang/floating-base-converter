...
def main():
    ...
    parser.add_argument("--from-base", "-f", type=int, choices=[2, 8, 10, 16],
                       default=10, help="Source base (default: 10)")
    parser.add_argument("--to-base", "-t", type=int, choices=[2, 8, 10, 16],
                       default=2, help="Target base (default: 2)")
    ...
    try:
        ...
        result = converter.convert(args.number, args.from_base, args.to_base, args.precision)
        
        base_names = {2: "binary", 8: "octal", 10: "decimal", 16: "hexadecimal"}
        print(f"{args.number} ({base_names[args.from_base]}) = {result} ({base_names[args.to_base]})")
